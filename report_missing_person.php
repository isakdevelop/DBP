...
if(isset($_SESSION['police'])){
  ...
  switch($search_terms){
    case 'title':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE title LIKE '%$search_value%'";
      break;
    case 'mpName':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE name LIKE '%$search_value%'";
      break;
    case 'writer':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE Missing_Person_Report.id = (SELECT id
       FROM user_infomation
       WHERE user_infomation.name = '".$search_value."')";
      break;
    case 'ID':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE id LIKE '%$search_value%'";
      break;
    default:
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report";
      break;
  }
...
//사용자 로그인
  ...
  switch($search_terms){
    case 'title':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE id = '$userId' AND title LIKE '%$search_value%'";
      break;
    case 'npName':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE id = '$userId' AND name LIKE '%$search_value%'";
      break;
    case 'writer':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE Missing_Person_Report.id
      = (SELECT id
         FROM user_infomation
         WHERE user_infomation.name = $name)";
      break;
    case 'ID':
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE id = '$userId' AND id LIKE '%$search_value%'";
      break;
    default:
      $sql = "SELECT idx, id, idx_ref_mp AS irp, name, content, title, pw FROM Missing_Person_Report WHERE id = '$userId'";
      break;
  }
...
<head>
    ...
    <link href="./css/report_missing_person.css" type="text/css" rel="stylesheet" >
    ...
    <div class="main_page">
        <div class="sub_frame">
            ...
            <section class="notice col-12 mx-auto">
                ...
                <form action="./report_missing_person.php" method="post">
                <select id="search_terms" name="search_terms" class="sel" style="width: 80px; text-align: center;">
                    <option value="title">제목</option>
                    <option value="mpName" name="mpName">실종자</option>
    ...
    <script>
      ...
      function modify_func(){
        ...
        if(ispol == true){
          ...
          if(result){
            location.href = "https://jj7932.cafe24.com/dbp/mp_modify_process.php?title="+title_value+"&content="+content_value+"&idx="+select_title_idx;
          }
        }
        else {
      ...
      function delete_func(){
        if(ispol == true){
          ...
          if(result){
            location.href = "https://jj7932.cafe24.com/dbp/mp_delete_process.php?idx="+select_title_idx;
          }
        }
        else {
          var pw_value = document.getElementById('password').value;
