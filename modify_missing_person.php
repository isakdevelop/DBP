...
$hair_shape = $_GET['hair_shape'] ?? '';
$cloth = $_GET['cloth'] ?? '';

$control_cnt = 0;
echo "<script>var taken_url = '$photo';</script>";
...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <div id="content">
      <form action="./modify_missing_person_process.php?idx=<?=$idx?>" method="post" name="form" enctype="multipart/form-data" novalidate>
        ...
        <span class="box_int_photo">
          ...
          <div id="image_container">
            <img id="thumbnail_img" src="<?=$photo?>" style="height:250px; width:200px;"/>
          </div>
        </span>
        <h3 class="join_title"><label for="name">성명<small> (필수)</small></label></h3>
        <span class="box int_name">
            <input type="text" id="name" name="name" class="int" maxlength="20" placeholder="홍길동" value="<?=$name?>">
        </span>
        ...
        <span class="box int_age">
            <input type="text" id="age" name="age" class="int" maxlength="3" placeholder="30" value="<?=$age?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
        </span>
        ...
        <span class="box nationality_code">
            <select id="nationality" name="nationality" class="sel">
                <option>국적</option>
                <option value="내국인" <?php if($nationality == "내국인") echo "selected";?>>내국인</option>
                <option value="외국인" <?php if($nationality == "외국인") echo "selected";?>>외국인</option>
            </select>
        </span>
        <h3 class="join_title"><label for="date_of_occurrence">발생 일자<small> (필수)</small></label></h3>
        <span class="box int_date_of_occurrence">
            <input type="text" id="date_of_occurrence" name="date_of_occurrence" class="int" maxlength="20" value="<?=$date?>" placeholder="1999년 12월 03일">
        </span>
        ...
        <span class="box int_address">
            <input type="text" id="address" name="address" value="<?=$address?>" class="int" maxlength="40">
        </span>
        ...
        <span class="box int_height">
            <input type="text" id="height" name="height" class="int" value="<?=$height?>" maxlength="3" placeholder="180" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
        </span>
        <h3 class="join_title"><label for="weight">몸무게</label></h3>
        <span class="box int_weight">
            <input type="text" id="weight" name="weight" value="<?=$weight?>" class="int" maxlength="3" placeholder="70" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
        </span>
        <h3 class="join_title"><label for="build">체형</label></h3>
        <span class="box int_build">
            <input type="text" id="build" name="build" class="int" value="<?=$build?>" maxlength="5" placeholder="보통">
        </span>
        <h3 class="join_title"><label for="face_shape">얼굴형</label></h3>
        <span class="box int_face_shape">
            <input type="text" id="face_shape" name="face_shape" value="<?=$face_shape?>" class="int" maxlength="5" placeholder="둥근형">
        </span>
        <h3 class="join_title"><label for="hair_color">두발 색상</label></h3>
        <span class="box int_hair_color">
            <input type="text" id="hair_color" name="hair_color" class="int" value="<?=$hair_color?>" maxlength="5" placeholder="흑색">
        </span>
        <h3 class="join_title"><label for="hair_shape">두발 형태</label></h3>
        <span class="box int_hair_shape">
            <input type="text" id="hair_shape" name="hair_shape" class="int" value="<?=$hair_shape?>" maxlength="10" placeholder="곱슬머리">
        </span>
        <h3 class="join_title"><label for="cloth">착의 의상</label></h3>
        <span class="box int_cloth">
            <input type="text" id="cloth" name="cloth" class="int" value="<?=$cloth?>" maxlength="10" placeholder="꽃무늬 난방">
        </span>
        <!-- JOIN BTN-->
        <div class="btn_area">
            <input type="button" id="btnJoin" value="이 전" onclick="location.href='./main.php'">
            </input>
            <input type="button" id="btnJoin" value="저 장" onclick="insert()">
    ...
    var address_ = document.getElementById('address');
        ...
        function insert(){
          ...
          face_shape = document.getElementById('build').value;
        ...
        reader.onload = function(event) {
          var beforeImg = document.getElementById('thumbnail_img');
          beforeImg.remove();
