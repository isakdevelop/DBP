...
if ($input_name == '') {
  $sql = "SELECT idx, photo, name, crime, Place_of_Registration, address FROM wanted";
}
else {
  $sql = "SELECT idx, photo, name, crime, Place_of_Registration, address FROM wanted WHERE name = '$input_name'";
}
...
while($row = mysqli_fetch_array($result)){
  ...
  $nameFromDB = $row['name'];
  $crimeFromDB = $row['crime'];
  $placeFromDB = $row['Place_of_Registration'];
...
<head>
    ...
    <link href="./css/criminal_info.css" type="text/css" rel="stylesheet" >
...
<body>
    ...
    <div class="main_page">
        <div class="sub_frame">
            <div class="missing_title">수배자 정보 게시판</div>
                ...
                { //만약 page가 1보다 크거나 같다면
                  ...
                  echo "<li class='fo_re'><a href='?page=1'>[처음]</a></li>"; //알니라면 처음글자에 1번페이지로 갈 수있게 링크
                   ...
                   { //만약 page가 1보다 크거나 같다면 빈값
                       ...
                       $pre = $page-3; //pre변수에 page-1을 해준다 만약 현재 페이지가 3인데 이전버튼을 누르면 2번페이지로 갈 수 있게 함
                   ...
                   if($block_num >= $total_block){ //만약 현재 블록이 블록 총개수보다 크거나 같다면 빈 값
                       ...
                       $next = $page + 3; //next변수에 page + 1을 해준다.
    ...
    <script>
      ...
      console.log(start, start+viewNum);
      for(var i = start; i<start+viewNum; i++){
        var send_name = mp_arr[i].name;
        var send_photo = mp_arr[i].photo;
        var send_crime = mp_arr[i].crime;
        var send_place = mp_arr[i].place;
        var send_address = mp_arr[i].address;
        $('#items').append(
          "<li id='item_"+mp_arr[i].idx+"' onclick='move_page(this)'>"+
            "<dl>"+
              "<dd><span class='gal-desc' style='text-align: center;'>"+mp_arr[i].name+"</span></dd>"+
              "<dd class='gal-thum'>"+
                "<div>"+
                  "<img class=img-responsive src='"+mp_arr[i].photo+"'/>"+
                "</div></dd>"+
                "<dd><span class='gal-title'>범죄명:</span>"+
                "<span class='gal-desc'>"+mp_arr[i].crime+"</span></dd>"+
                "<dd><span class='gal-title'>등록지:</span>"+
                "<span class='gal-desc'>"+mp_arr[i].place+"</span></dd>"+
                "<dd><span class='gal-title'>주소:</span>"+
      ...
      function find_person(){
        ...
        location.href = "https://jj7932.cafe24.com/dbp/criminal_info.php?input_name="+input_name;
      }
      function move_page(obj){
        var splitID = (obj.id).split('_');
        var index = splitID[1];
        location.href = "https://jj7932.cafe24.com/dbp/criminal_data.php?idx="+index;
      }
